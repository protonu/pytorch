...
TEST(TensorIndexingTest, TestNone) {
  auto v = torch::randn({5, 7, 3});
  ASSERT_EQ(v.idx({None}).sizes(), {1, 5, 7, 3});
  ASSERT_EQ(v.idx({{}, None}).sizes(), {5, 1, 7, 3});
  ASSERT_EQ(v.idx({{}, None, None}).sizes(), {5, 1, 1, 7, 3});
  ASSERT_EQ(v.idx({"...", None}).sizes(), {5, 7, 3, 1});
}
...
TEST(TensorIndexingTest, TestStepAssignment) {
  ...
  ASSERT_TRUE(exactly_equal(v.idx({{1, None}}).sum(), 0));
}
...
TEST(TensorIndexingTest, TestBoolIndices) {
  {
    ...
    ASSERT_EQ(v.idx({boolIndices}).sizes(), {3, 7, 3});
    assert_tensor_equal(v.idx({boolIndices}), torch::stack({v(0), v(2), v(3)}));
  }
  {
